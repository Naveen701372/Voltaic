...
export interface DevServerInfo {
    ...
    url?: string;
    previewContent?: string; // Store HTML content for direct serving
}
...
<body>
    ...
    <script type="text/babel">
        ...
        try {
            ...
            console.log('Successfully executed imported components');
        } catch (error) {
            console.error('Failed to execute imported components:', error);
        }
        ...
        try {
            ...
            console.log('Successfully executed main component');
        } catch (error) {
            console.error('Failed to execute main component:', error);
        }
        ...
        // Extract the main component name (should be a capitalized name)
        const mainComponentName = Array.from(mainComponents)
            .find(name => /^[A-Z]/.test(name)) || null;
        ...
        function DefaultApp() {
            return React.createElement("div", { className: "p-4" },
                ...
                React.createElement("p", { className: "text-gray-600" },
                    "No app component found in the uploaded files.")
            );
        }
        ...
        function AppWrapper() {
            ...
            return React.createElement("div", {
                ...
                React.createElement("div", {
                    ...
                    }, "Generated by Voltaic • Running in Production Serverless Environment"),
                    React.createElement("div", {
                        style: {
                ...
                MainComponent ? React.createElement(MainComponent, null) :
                    window.Greeting ? React.createElement(window.Greeting, null) :
                        React.createElement(DefaultApp, null)
            );
        }
        ...
        root.render(React.createElement(AppWrapper, null));
                ...
                // Store the content for direct serving
                serverInfo.previewContent = htmlContent;
                serverInfo.status = 'running';
                serverInfo.url = `/api/dev-server/preview/${serverInfo.projectId}`;

                serverInfo.logs.push(`[${new Date().toISOString()}] Quick preview ready at ${serverInfo.url}`);
                serverInfo.logs.push(`[${new Date().toISOString()}] Content stored for direct serving (${htmlContent.length} chars)`);

                resolve();

            } catch (error) {
                const errorMsg = `Failed to create quick preview: ${error instanceof Error ? error.message : String(error)}`;
...
`;
    ...
    /**
     * Get preview content for a project
     */
    getPreviewContent(projectId: string): string | null {
        const serverInfo = this.runningServers.get(projectId);
        return serverInfo?.previewContent || null;
    }
} 